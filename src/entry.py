...
def generate_two_jsonfiles(dict_out1,file_name):
    dict_out = copy.deepcopy(dict_out1)
    dict1 = {}
    if "Roll" in dict_out:
        dict1["roll_no"] = dict_out["Roll"]
        del dict_out["Roll"]
    if "Centre" in dict_out:
        dict1["centre_no"] = dict_out["Centre"]
        del dict_out["Centre"]
    if "Book" in dict_out:
        dict1["test_booklet_no"] = dict_out["Book"]
        del dict_out["Book"]

    json_object1 = json.dumps(dict1, indent=4)
    
    with open("json_output_1_"+file_name+".json", "w") as outfile:
        outfile.write(json_object1)

    json_object2 = json.dumps(dict_out, indent=4)
    with open("json_output_2_"+file_name+".json", "w") as outfile:
...
def process_files(
    ...
    for file_path in omr_files:
        ...
        generate_two_jsonfiles(omr_response,str(file_name))
        # json_object = json.dumps(omr_response, indent=4)
        # # Writing to sample.json
        
        # current_out_file = out_file + str(file_name) + ".json"
        # with open(current_out_file, "w") as outfile:
